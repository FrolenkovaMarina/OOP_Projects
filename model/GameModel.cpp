...
// игровая модель (MVC): тут хранится состояние игры и вся логика обновления мира
// view рисует, Controller вызывает методы движения, а GameModel считает коллизии и генерирует полосы



// просто ограничиваем колонку в заданных границах [min_c; max_c]
// нужна, чтобы путь не уезжал слишком далеко влево/вправо
...
// конструктор: задаём размеры поля, стартовую позицию игрока и начальные параметры камеры
GameModel::GameModel()
...
{
    // инициализируем рандом для генерации полос/объектов
    std::srand((unsigned)std::time(nullptr));

    // создаём начальный набор полос (трава/дороги/реки) вокруг игрока
    initLanes();
}

// создаём стартовый набор полос
// мы идём от низа экрана к верху и добавляем Lane'ы в deque
void GameModel::initLanes()
{
    lanes_.clear();

    // с какой строки стоит игрок (ниже неё делаем безопасную зону травы)
    int start_row = player_.getRow();
    int road_streak = 0;
    int river_streak = 0;

    // path_open_cols_ — список проходимых колонок на траве
    // его используем, чтобы не генерировать тупики из кустов/камней
    ...
    for (int logical_row = height_ - 1; logical_row >= -1; --logical_row)
    {
        ...
        // ниже/на уровне игрока делаем в основном траву, чтобы старт был честный и без смерти сразу
        ...
        // с вероятностями решаем: будет река, дорога или трава
        // частоты подобраны просто на глаз, чтобы карта была разнообразной
        bool want_river = (std::rand() % 5 == 0);
        bool want_road = (std::rand() % 3 == 0);

        // ограничиваем серии: максимум 3 дороги подряд и максимум 3 реки подряд
        // и ещё: если уже выбрали реку, дорогу в этот раз не делаем
...
void GameModel::addNewTopLane()
{
    ...
    // считаем, сколько одинаковых полос подряд уже есть наверху
    int river_streak = 0;
    int road_streak = 0;

    // идём сверху вниз по текущим полосам, пока идут реки/дороги подряд
    // проверка столкновений с машинами, если игрок на RiverLane — он должен стоять на бревне
...
// cells может быть дробным (зависит от dt), а когда набирается >= 1 — сдвигаем на целую клетку
void GameModel::scrollWorld(float cells, bool movePlayerWithWorld)
{
    ...
    while (world_offset_ >= 1.f)
    {
        ...
        // сверху добавляем новую
        addNewTopLane();
    }
}

// прыжок вверх. Если дошли до замка (LOCK_ROW), дальше двигаем не игрока, а мир
...
// главный апдейт модели (вызывается каждый кадр)
// тут: обновление игрока/полос, автопрокрутка, проверки на смерть (машины/река)
void GameModel::update(float dt)
{
    if (state_ != GameState::Running)
        return;

    // обновляем игрока и все полосы (внутри полос двигаются машины/брёвна и т.п.)
    player_.update(dt);
    for (auto& lane : lanes_) {
        lane->update(dt);
    }

    // автопрокрутка мира (как в Crossy Road: если стоять, тебя уезжает вниз)
    ...
    for (std::size_t i = 0; i < lanes_.size(); ++i)
    {
        ...
        for (const auto& carPtr : cars)
        {
            ...
            // простая AABB-проверка пересечения прямоугольников
...
// проверка для травы: есть ли в клетке препятствие (куст/камень и т.п.)
...
// перезапуск игры: возвращаем игрока на старт и заново генерируем мир
...
// path_open_cols_ хранит 1 или 2 колонки (иногда делаем проход шире)
...
// важно: на строке игрока принудительно добавляем его колонку, чтобы не заспавнить препятствие под ним
